...
const run = async () => {
    try {
        ...
        let promise = service.getPosisjonForVegsystemreferanse("ev18s1d1m200");

        promise.then(async posisjon => {
            const vegobjektResponse = await service.fetchHistoricVegreferanseByLenkeposisjon(posisjon.veglenkesekvens.veglenkesekvensid, posisjon.veglenkesekvens.relativPosisjon);
